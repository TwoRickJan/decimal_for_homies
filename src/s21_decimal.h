...
int s21_get_bit(s21_decimal d, int index);        // Получение бита по индексу
void s21_set_bit(s21_decimal *d, int index, int bit);// Установка бита по индексу
...
void s21_right_shift_mantissa(s21_decimal *d, int shift);// Сдвиг мантиссы вправо на указанное количество разрядов
...
int s21_is_greater_mantissa(s21_decimal a, s21_decimal b);        // Проверка больше для мантисс
int s21_is_less_mantissa(s21_decimal a, s21_decimal b);          // Проверка меньше для мантисс
